...
class MongoDBHandler:
    ...
    def __init__(self):
        ...
        #config.read('C:/stock-trading-master/conf/config.ini')
        #host = config['MONGODB']['host']
        #port = config['MONGODB']['port']
        #self._client = MongoClient(host, int(port))
        self._client = MongoClient("mongodb://stockt1:OAMGwGLhOGcYitd2TLHuPFdmslT0lCeHjbB8R1fPwbS9PCjF71hy3uMPHVeoCYZtKQrnLtAslpDSCE2LNio3Sg==@stockt1.mongo.cosmos.azure.com:10255/?ssl=true&replicaSet=globaldb&retrywrites=false&maxIdleTimeMS=120000&appName=@stockt1@")
    ...
    def find_items(self, condition=None, db_name=None, collection_name=None):
        ...
        if db_name is None or collection_name is None:
            raise Exception("Need to param db_name, collection_name")
        return self._client[db_name][collection_name].find(condition, {"_id": False})
        
    def find_items_column(self, condition=None, column=None, db_name=None, collection_name=None):
        if condition is None or not isinstance(condition, dict):
            condition = {}
        if column is None or not isinstance(column, dict):
            column = {}
        if db_name is None or collection_name is None:
            raise Exception("Need to param db_name, collection_name")
        return self._client[db_name][collection_name].find(condition, column, no_cursor_timeout=True, cursor_type=CursorType.EXHAUST)
